...
financeControllers.controller('ImportController', function ($scope, $modal, CSV, TransactionService, BudgetService) {
  ...
  $scope.parseCSV = function () {
    initVars();
    if (!$scope.csvContent) { return; }

    var objs = CSV.CSVToArray($scope.csvContent);

    // Auto-detect columns based on header or content
    // We assume order: Date, Amount, Remark, [Category]

    $scope.parsedTransactions = objs.map(function (row) {
      if (row.length < 3) return null; // Skip empty/short lines

      var dateStr = row[0];
      var amountStr = row[1];
      var remark = row[2];
      var categoryName = row.length > 3 ? row[3] : null;

      var amount = Math.abs(parseFloat(amountStr));
      if (isNaN(amount)) { amount = 0; } // Handle header row gracefully by failing validation later

      var date = new Date(dateStr);

      // Try to match category automatically
      var budget = null;
      if (categoryName && $scope.budgets) {
        var match = _.find($scope.budgets, function (b) {
          return b.name.toLowerCase() === categoryName.trim().toLowerCase();
        });
    ...
    }).filter(function (t) { return t !== null && !isNaN(t.transactionOn.getTime()); }); // Filter out bad rows

    $scope.columnsConfirmed = true;
    $scope.transactionsLoaded = true;
    $scope.transactions = $scope.parsedTransactions;
  }

  // Pre-fetch budgets for auto-matching
  BudgetService.monthly({ month: new Date().getMonth() + 1, year: new Date().getFullYear() }, function (response) {
    $scope.budgets = response;
  });
